...
void ControllerComponent::Initialize(const SceneContext& /*sceneContext*/)
{
	if(!m_IsInitialized)
	{
		auto p = m_pGameObject->GetComponent<TransformComponent>()->GetPosition();
		m_ControllerDesc.position.x = p.x;
		m_ControllerDesc.position.y = p.y;
		m_ControllerDesc.position.z = p.z;
		m_ControllerDesc.userData = this;
		
		auto controllerManager = m_pScene->GetPhysxProxy();
		m_pController = controllerManager->GetControllerManager()->createController(m_ControllerDesc);
