...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	//		*nearZ>0.1f
	//		*farZ>500.f
	XMMatrixOrthographicLH(100.f * m_GameContext.windowWidth / m_GameContext.windowHeight, 100.f, 0.1f, 500.f);
	...
	auto focusPos = lightPos + lightDir;

	auto viewMatrix = XMMatrixLookAtLH(lightPos, focusPos, {0,1,0});
	//- Use the Projection & View Matrix to calculate the ViewProjection of this Light, store in m_LightVP

	XMStoreFloat4x4(&m_LightVP, 
		XMMatrixMultiply(viewMatrix, XMLoadFloat4x4(&sceneContext.pCamera->GetProjection()))
	);
