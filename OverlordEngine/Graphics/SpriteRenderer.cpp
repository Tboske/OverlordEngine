...
void SpriteRenderer::UpdateBuffer(const SceneContext& sceneContext)
{
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		m_BufferSize = (m_Sprites.size() > m_BufferSize) ? UINT(m_Sprites.size()) : m_BufferSize;
	...
	if (m_pVertexBuffer)
	{
		...
		m_GameContext.d3dContext.pDeviceContext->Map(m_pVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
		// use memcpy to copy all our sprite vertices (m_Sprites) to the mapped resource (D3D11_MAPPED_SUBRESOURCE::pData)
		//memcpy(&mappedSubresource, &m_Sprites, sizeof(m_Sprites));
		mappedSubresource.pData = m_Sprites.data();
