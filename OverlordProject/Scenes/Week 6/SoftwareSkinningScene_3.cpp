...
void SoftwareSkinningScene_3::Update()
{
	...
	for (size_t i{ 0 }; i < m_SkinnedVertices.size(); ++i)
	{
		...
		auto tr0 = XMVector3TransformCoord(vector, boneTransform0);
		auto tr1 = XMVector3TransformCoord(vector, boneTransform1);

		XMStoreFloat3(&m_SkinnedVertices[i].transformedVertex.Position , XMVectorLerp(tr0, tr1, m_SkinnedVertices[i].blendWeight1));
	}
...
void SoftwareSkinningScene_3::InitVertices(float length)
{
