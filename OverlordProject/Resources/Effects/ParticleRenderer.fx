...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	topLeft		= mul(float4(topLeft	, 0.f), gViewInverse).xyz;
	topRight	= mul(float4(topRight	, 0.f), gViewInverse).xyz;
	bottomLeft	= mul(float4(bottomLeft	, 0.f), gViewInverse).xyz;
	bottomRight = mul(float4(bottomRight, 0.f), gViewInverse).xyz;
